...
def frames_to_vid(save_to_dir, fps):
    ...
    if len(os.listdir(os.getcwd())) == 0:
       logging.info(f"No bees were found in video '{os.path.split(save_to_dir)[1]}.h264'") 
        ...
        logging.info(f"Reconstructing frames to video for '{os.path.split(save_to_dir)[1]}.h264'")
...
def yolo_detect(model_weights, batch_size, image_list, save_to_dir, extract_timestamp, cut, window_size):
    ...
    for image in image_list:
        ...
        if i % batch_size == 0 or i == len(image_list):
            ...
            if len(imgs) == batch_size:
                ...
                bees_detected_dir, bees_df = move_detected_objects_to_dir(batch_pred, save_to_dir, extract_timestamp=extract_timestamp, cut=cut, bees_df=bees_df, ocr=ocr)
    ...
    logging.info(f"CSV file generated for '{os.path.split(save_to_dir)[1]}.h264'")
...
def move_detected_objects_to_dir(pred, save_to_dir, **kwargs):
    """
    ...
    """
    if kwargs:
        extract_timestamp = kwargs['extract_timestamp']  
        cut = kwargs['cut']
    ...
    #Column Names: xmin    ymin    xmax    ymax    confidence  class_number   class_name    
        np.savetxt(f'{bees_detected_dir}/{os.path.splitext(os.path.split(pred.files[index])[1])[0]}.txt', df.values, fmt='%s')
        bees_df = bees_dataframe(df, f'{save_to_dir}/bees_detected/{pred.files[index]}', extract_timestamp, bees_df, ocr)
...
def process_video(video_path):
    ...
    logging.basicConfig(filename = f"{save_to_dir}/outputs/run.log", format = '%(asctime)s | %(message)s', filemode = "w", level=logging.INFO, force=True)
